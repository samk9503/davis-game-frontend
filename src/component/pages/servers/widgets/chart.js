...
const Chart = (props) => {
  ...
  useEffect(() => {
    const getData = () => {
      ...
      axios
        .get(
          "https://api.battlemetrics.com/servers/"+props.id+"/"+props.url+"?start=" +
            start +
            "&stop=" +
            stop
        )
      ...
      setOptionsServer({
        ...
        scales: {
          yAxes: {
            ...
            min: props.url=='rank-history'?Math.min(...valuesServer):Math.min(...valuesServer)-Math.max(...valuesServer)*.05,
            max: props.url=='rank-history'?Math.max(...valuesServer):Math.max(...valuesServer)+Math.max(...valuesServer)*.05,
            ...
            ticks: {
              ...
              color: "white",
              min: 1,
              stepSize: props.url=='rank-history'?Math.max(...valuesServer)*.4:Math.max(...valuesServer)*.1,
              font: {
                size: 8,
                weight: "50",
                lineHeight: 0,
